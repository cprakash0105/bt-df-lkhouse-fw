...
def list_deployed_dataproducts():
    ...
    try:
        ...
        products = []
        for b in blobs:
            if b.name.endswith(".deployment.json"):
                try:
                    record = json.loads(b.download_as_text())
...
def _fetch_schema_from_landing(dataset_name: str) -> Optional[dict]:
    ...
    try:
        ...
        # Read the most recently updated file (skip folder markers)
        real_blobs = [b for b in blobs if b.size and b.size > 0 and not b.name.endswith('/')]
        if not real_blobs:
            return None
        blob = max(real_blobs, key=lambda b: b.updated)
