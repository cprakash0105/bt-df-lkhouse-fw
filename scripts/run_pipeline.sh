...
if [[ "$STAGE" == "all" || "$STAGE" == "ingest" ]]; then
  ...
  gcloud dataproc batches submit pyspark \
    ...
    --py-files=${PY_FILES} \
    --properties="spark.jars.packages=org.apache.iceberg:iceberg-spark-runtime-3.5_2.12:1.9.1,spark.executor.instances=0,spark.dynamicAllocation.enabled=false" \
