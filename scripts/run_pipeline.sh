...
if [[ "$STAGE" == "all" || "$STAGE" == "ingest" ]]; then
  ...
  gcloud dataproc batches submit pyspark \
    ...
    --py-files=${PY_FILES} \
    --properties="spark.jars.packages=org.apache.iceberg:iceberg-spark-runtime-3.5_2.12:1.9.1,spark.executor.instances=2,spark.driver.cores=4,spark.executor.cores=4,spark.driver.memory=4g,spark.executor.memory=4g" \
