#!/bin/bash
# EastSide CDH 2.0 — Run Gold Engine on Dataproc (lakehouse-cluster)
# Usage: bash eastside/scripts/run_gold.sh [all|table_name]
...
echo "============================================================"
echo "  EastSide Gold — lakehouse-cluster"
echo "  Target: ${TARGET}"
echo "============================================================"
...
gcloud dataproc jobs submit pyspark gs://${BUCKET}/engine/gold.py \
    --cluster=${CLUSTER} \
    --project=${PROJECT} \
    --region=${REGION} \
    --py-files=gs://${BUCKET}/engine/base.py,gs://${BUCKET}/engine/schema_evolver.py \
    --jars=gs://bt-df-lkhouse-lakehouse/spark/iceberg-spark-runtime.jar,gs://bt-df-lkhouse-lakehouse/spark/biglake-catalog.jar,gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.12-0.36.1.jar \
    --properties="\
